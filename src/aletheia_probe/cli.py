...
def _persist_candidate_mappings(
...
async def _build_publication_candidates(
...
def _resolve_candidate_selection(
    ...
    if _is_acronym_like_candidate(selection_result.best_label, best_query_text):
...
def _finalize_publication_result(
    result: AssessmentResult,
    publication_name: str,
    assessed_candidates: list[tuple[str, AssessmentResult, QueryInput]],
    selection_result: CandidateSelectionResult,
...
async def _async_assess_publication(
    ...
    try:
        ...
        candidate_build_result = await _build_publication_candidates(
        ...
        _persist_candidate_mappings(candidates, acronym_cache)
        ...
        assessed_candidates = await _assess_candidates(candidates)
        selection_result = _resolve_candidate_selection(
