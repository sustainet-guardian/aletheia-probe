...
\textbf{Evidence-Based Decision Support.} Aletheia-Probe provides
comprehensive evidence and transparent reasoning that researchers use
to make informed decisions according to their institutional policies
and risk tolerance. Rather than issuing binary directives to ``accept''
or ``reject'' journals, the tool presents all available evidence---what
each source found, how confident each assessment is, where sources
agree or conflict---enabling researchers to understand the basis for
classification. This approach respects the reality that appropriate
decisions depend on context: a journal flagged with moderate confidence
might be acceptable for one institution's bibliography but warrant
additional scrutiny for another's publication venue selection.
Researchers should evaluate the presented evidence alongside their
domain expertise, institutional guidelines, and specific use case
requirements. This evidence-based approach makes assessments more
trustworthy precisely because researchers understand what they're
acting upon rather than blindly following opaque recommendations.
...
\textbf{Point-in-Time Assessments.} As discussed in Section 3.4,
assessments reflect data source state at query time. A journal assessed
today may receive a different classification in six months if data
sources update. Results are not permanent verdicts but snapshots of
current evidence.

\textbf{No Substitute for Institutional Policy.} The tool provides
evidence-based assessments according to aggregated data sources, but
institutions may impose stricter or different criteria. A journal
classified as legitimate by the tool may still violate institutional
policies regarding publication fees, editorial board composition, or
disciplinary focus.

These limitations are inherent to the aggregation approach and data
dependency.
