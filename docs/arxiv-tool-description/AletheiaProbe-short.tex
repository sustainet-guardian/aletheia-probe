...
\hypersetup{
  ...
  linkcolor=color1,
  bookmarksopen=false,
  pdftitle={Aletheia-Probe: A Tool for Automated Journal Assessment},
  pdfauthor={Andreas Florath}
}

\JournalInfo{V1}
\Archive{}
\PaperTitle{Aletheia-Probe: A Tool for Automated Journal Assessment}
...
\end{tikzpicture}
\caption{Aletheia Probe Data Flow Architecture: Multi-stage pipeline
  for journal legitimacy assessment combining curated lists, API
  analyzers, and pattern heuristics.}
...
query orchestrator sends queries to all backends simultaneously.
...
and Space (BMFTR) in Germany under grant number 16KIS2240 of the
...
The author declares no conflicts of interest. Language editing was
