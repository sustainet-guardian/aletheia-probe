...
def test_included_group1_source_basics(mocked_config):
    """Test basic source metadata for included Group I side."""
    source = UgcCareIncludedFromCloneGroup1Source()
    assert source.get_name() == "ugc_care_included_from_clone_group1"
    assert source.get_list_type() == AssessmentType.LEGITIMATE


def test_included_group2_source_basics(mocked_config):
    """Test basic source metadata for included Group II side."""
    source = UgcCareIncludedFromCloneGroup2Source()
    assert source.get_name() == "ugc_care_included_from_clone_group2"
    assert source.get_list_type() == AssessmentType.LEGITIMATE
...
def test_parse_cloned_list_records(mocked_config):
    ...
    assert entries[0]["issn"] == "3333-4444"
    assert entries[0]["metadata"]["original_title"] == "Original Journal"
    assert entries[1]["issn"] == "7777-8888"
...
def test_parse_included_group1_list_records(mocked_config):
    """Test Group I included-side parser returns left side entries."""
    source = UgcCareIncludedFromCloneGroup1Source()
    html = """
    <div>
      1 Title - Original Journal URL : http://orig.example
    ...
    assert len(entries) == 2
    assert entries[0]["journal_name"] == "Original Journal"
    assert entries[0]["metadata"]["ugc_status"] == "included_group_i_from_clone_page"
    assert entries[0]["issn"] == "1111-2222"
    assert entries[1]["journal_name"] == "Cloned Journal"
    assert entries[1]["issn"] == "3333-4444"


def test_parse_included_group2_list_records(mocked_config):
    """Test Group II included-side parser returns left side entries."""
    source = UgcCareIncludedFromCloneGroup2Source()
    html = """
    <div>
      1 Title - Original Two URL : http://orig.example
      Publisher : Real Pub ISSN : 8888-1111
      Title - Cloned Two URL : http://clone.example
      Publisher : Clone Pub ISSN : 9999-0000
    </div>
    """
    ...
    assert len(entries) == 2
    assert entries[0]["journal_name"] == "Original Two"
    assert entries[0]["metadata"]["ugc_status"] == "included_group_ii_from_clone_page"
    assert entries[0]["issn"] == "8888-1111"
    assert entries[1]["journal_name"] == "Cloned Two"
    assert entries[1]["issn"] == "9999-0000"
