...
class TestRetractionWatchSource:
    ...
    def source(self):
        ...
        with patch("aletheia_probe.config.get_config_manager") as mock_config_manager:
            ...
            mock_config.data_source_urls.retraction_watch_repo_url = "https://gitlab.com/test/retraction-watch-data.git"
            mock_config.data_source_urls.retraction_watch_csv_filename = "retraction_watch_data.csv"
    ...
    def test_should_update_no_last_update(self, source):
        ...
        with patch("aletheia_probe.updater.sources.retraction_watch.get_cache_manager") as mock_get_cache_manager:
            mock_cache = Mock()
    ...
    async def test_clone_repository_success(self, source):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("subprocess.run", side_effect=lambda *args, **kwargs: mock_run_git_clone()):
    ...
    def test_clone_repository_invalid_temp_dir(self, source):
        ...
        try:
            result = loop.run_until_complete(source._clone_repository("/nonexistent/path"))
            assert result is None
        finally:
            loop.close()
    ...
    def test_clone_repository_temp_dir_not_directory(self, source):
        ...
        with tempfile.NamedTemporaryFile() as temp_file:
            ...
            asyncio.set_event_loop(loop)
            try:
                result = loop.run_until_complete(source._clone_repository(temp_file.name))
                assert result is None
            finally:
                loop.close()
    ...
    def test_calculate_risk_level_no_publications(self, source):
        ...
        assert source._calculate_risk_level(6, 2) == "moderate"  # RETRACTION_COUNT_MODERATE = 6
        assert source._calculate_risk_level(11, 5) == "high"  # RETRACTION_COUNT_HIGH = 11
    ...
    def test_calculate_risk_level_with_publications(self, source):
        ...
        assert source._calculate_risk_level(5, 2, 1000, 500) == "low"  # 0.5% overall rate
        # 20/1000 = 2.0%, which is > RETRACTION_RATE_HIGH (1.5%) but < RETRACTION_RATE_CRITICAL (3.0%)
        assert source._calculate_risk_level(20, 10, 1000, 500) == "high"  # 2.0% overall rate
    ...
    async def test_parse_and_aggregate_csv_simple(self, source):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False, encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=[
                'Journal', 'Publisher', 'RetractionDate', 'RetractionNature',
                'Reason', 'OriginalPaperDOI', 'RetractionDOI'
            ])
            writer.writeheader()
            writer.writerow({
        ...
        try:
            with patch.object(source, "_batch_cache_article_retractions", new=AsyncMock()):
    ...
    async def test_parse_and_aggregate_csv_normalization_failure(self, source):
        ...
        try:
            with (
                patch.object(source, "_batch_cache_article_retractions", new=AsyncMock()),
                patch("aletheia_probe.normalizer.input_normalizer.normalize", side_effect=Exception("Normalization failed"))
            ):
                result = await source._parse_and_aggregate_csv(csv_path)
    ...
    async def test_parse_and_aggregate_csv_multiple_journals(self, source):
        ...
        try:
            ...
            test_journal = next(j for j in result if j["journal_name"] == "Test Journal")
            assert test_journal["metadata"]["total_retractions"] == 2
            assert test_journal["metadata"]["recent_retractions"] == 1  # Only 2022 is > 2 years ago
            assert test_journal["metadata"]["very_recent_retractions"] == 0  # None in last year
            ...
            top_reasons = [reason[0] for reason in test_journal["metadata"]["top_reasons"]]
    ...
    async def test_parse_and_aggregate_csv_empty_journal_name(self, source):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False, encoding='utf-8') as f:
            ...
            writer.writerow({'Journal': 'Valid Journal', 'OriginalPaperDOI': '10.1234/valid.doi'})
            csv_path = Path(f.name)
    ...
    async def test_clone_repository_path_traversal_protection(self, source):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert result is None or result.resolve().is_relative_to(temp_directory.resolve())