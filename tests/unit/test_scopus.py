...
class TestScopusSource:
    ...
    def test_find_column_indices(self):
        """Test _find_column_indices finds correct column mappings."""
        source = ScopusSource()
        headers = [
            "Source Title",
            "ISSN",
        ...
        col_indices = source._find_column_indices(headers)

        assert col_indices["title"] == 0
        assert col_indices["issn"] == 1
        assert col_indices["eissn"] == 2
        assert col_indices["publisher"] == 3
        assert col_indices["status"] == 4
        assert col_indices["source_type"] == 5

    def test_find_column_indices_case_insensitive(self):
        """Test _find_column_indices is case-insensitive."""
        source = ScopusSource()
        headers = ["source title", "issn", "eissn"]
    ...
    def test_validate_and_normalize_issn_valid(self):
        """Test _validate_and_normalize_issn with valid ISSN."""
        source = ScopusSource()

        # Test with hyphenated ISSN
        result = source._validate_and_normalize_issn("1234-5679", "Test Journal")
        assert result == "1234-5679"

        # Test with non-hyphenated ISSN (should add hyphen)
        result = source._validate_and_normalize_issn("12345679", "Test Journal")
        assert result == "1234-5679"

    def test_validate_and_normalize_issn_invalid(self):
        """Test _validate_and_normalize_issn with invalid ISSN."""
        source = ScopusSource()

        result = source._validate_and_normalize_issn("INVALID", "Test Journal")
        assert result is None

    def test_validate_and_normalize_issn_none(self):
        """Test _validate_and_normalize_issn with None."""
        source = ScopusSource()

        result = source._validate_and_normalize_issn(None, "Test Journal")
        assert result is None

    def test_create_journal_entry_success(self):
        """Test _create_journal_entry creates valid entry."""
        source = ScopusSource()

        with patch("aletheia_probe.normalizer.input_normalizer.normalize") as mock_norm:
            mock_norm.return_value = Mock(normalized_name="test journal")
            ...
            assert entry["normalized_name"] == "test journal"
            assert entry["issn"] == "1234-5679"
            assert entry["eissn"] == "2049-3630"
            assert entry["publisher"] == "Test Publisher"
            assert entry["metadata"]["source_type"] == "Journal"
            assert entry["metadata"]["coverage"] == "2020-2024"
            assert entry["metadata"]["open_access"] == "Open Access"
            assert "quality_flagged" not in entry["metadata"]

    def test_create_journal_entry_with_quality_flag(self):
        """Test _create_journal_entry with quality flag."""
        source = ScopusSource()

        with patch("aletheia_probe.normalizer.input_normalizer.normalize") as mock_norm:
            mock_norm.return_value = Mock(normalized_name="flagged journal")
            ...
            entry = source._create_journal_entry(
                ...
                quality_flag="Poor editorial practices",
            )
